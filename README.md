...
## 주요 기능
- **REFPOLY**: 기준 폴리라인 구간(`P1-P2`)을 참조 구간(`Q1-Q2`)으로 치환합니다.
  - `F2P`: 기준/참조 모두 시계(CW) 기준
  - `F3P`: 기준은 시계(CW) 유지, 참조는 반시계(CCW) 기준
- **ODLabel**: 객체 데이터(Object Data)를 추출하여 스마트한 위치와 각도로 레이블을 생성합니다.
- **AreaAdjust**: 지적 공차 공식을 적용하여 폴리라인의 면적을 정밀하게 조정합니다. (구간 이동 및 고정점 유지 오프셋 지원)
...
- **ArcToPolyline**: 호(Arc)를 현분 형태의 폴리라인으로 변환합니다. (분할, 중앙종거, 현 길이 옵션)
- **CoordRound**: 좌표를 오사오입(Round half to even) 규칙에 따라 정결합합니다.
- **VertexEdit**: 폴리라인의 정점을 직접 추가하거나 삭제합니다.
  - `VAD`: 클릭 좌표를 가장 가까운 선분 사이에 삽입
  - 기존 정점과 `0.0005m` 이내 클릭 시 추가하지 않음
...
## 단축 명령어 (Aliases)
- **F2P / F3P**: 기준 구간(`P1-P2`)을 참조 구간(`Q1-Q2`)으로 치환
  - `F2P`: 기준/참조 모두 시계(CW)
  - `F3P`: 기준 시계(CW), 참조 반시계(CCW)
- **ODL**: OD 데이터 레이블 생성
