...
## 개요
이 프로젝트는 AutoCAD Map 3D 2013 (Windows 11 환경)에서 발생할 수 있는 프리징 현상을 방지하고, 지적 측량 및 도면 작업의 효율성을 높이기 위해 개발된 리습(Lisp) 도구 모음입니다. 모든 도구는 명령행(Prompt) 인터페이스를 사용하여 안정성을 확보하였습니다.
...
- **ParcelSplit**: 지번 레이어를 분석하여 본번/부번, 지목, 대장구분 레이어로 분해합니다.
...
- **ArcToPolyline**: 호(Arc)를 선분 형태의 폴리라인으로 변환합니다. (수량, 중앙종거, 등간격 옵션)
- **CoordRound**: 좌표를 오사오입(Round half to even) 규칙에 따라 재결정합니다.
